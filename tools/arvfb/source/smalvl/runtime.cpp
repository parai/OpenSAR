...
runtime_t::runtime_t(): _memmanager( heap_manager_t::get_instance() )
{
...
function_declaration_t* runtime_t::get_function_declaration(std::string name) throw (runtime_exception_t) {
    try {
        return _function_declarations.at(name);
    }
    catch(std::out_of_range e)
    {
        RUNTIME_DEBUG("Function '%s' don't exist in the global scope", name.c_str());
        throw runtime_exception_t(FUNCTION_NOT_DECL);
    }
}

void runtime_t::add_function_declaration(function_declaration_t* fd) throw (runtime_exception_t) {
    ...
    {
        std::cerr << "Semantic error: Function " << fd->get_name() << " redeclaration" << std::endl;
    }
}
...
ref_t runtime_t::run_function(function_call_t* fc, frame_stack_t& fs) throw(runtime_exception_t) {
    ...
    try {
        ...
        for(; decl_args!= fun->get_args().end() && call_args!=fc->get_args().end(); decl_args++, call_args++)
        {
...
ref_t runtime_t::run(block_t* block, frame_stack_t& fs) throw(runtime_exception_t) {
...
ref_t runtime_t::run_instruction(oper_t* instruction, frame_stack_t& fs) throw(runtime_exception_t) {
#define ti typeid(*instruction)
	...
	run_instruction(for_op->get_init_op(), fs);
        ...
        object_t* condition_obj = _memmanager->get_object( compute_expression(while_op->get_condition(),fs) );

        for(; (bool) (*condition_obj);
                condition_obj = _memmanager->get_object( compute_expression(while_op->get_condition(),fs) ) )
        {
            RUNTIME_DEBUG("While ( %s ) cycle", ((const char*) *condition_obj));
        ...
        object_t* condition_obj = _memmanager->get_object(compute_expression(if_op->get_condition(), fs));
        RUNTIME_DEBUG("If/else ( %s ) statement", ((const char*) *condition_obj));
...
ref_t runtime_t::compute_expression(expr_t* expr, frame_stack_t& fs) throw (runtime_exception_t) {
    if(typeid(*expr)==typeid(binary_t)) {
        /* Math operations */
        binary_t* bin = (binary_t*) (expr);
        object_t* arg1 = _memmanager->get_object( compute_expression(bin->get_operand(0), fs) );
        object_t* arg2 = _memmanager->get_object( compute_expression(bin->get_operand(1), fs) );
...
ref_t runtime_t::compute_math_expression(object_t* obj1, const char* op, object_t* obj2)
throw (runtime_exception_t, std::invalid_argument) {
    ...
    if( op[0] == '.' )
    {
        RUNTIME_DEBUG("\tstring '%s' and '%s' concetate", ((std::string) *obj1).data(), ((std::string) *obj2).data());
     ...
     */
    else if( (strchr("+-/%*", op[0])!=NULL) && (obj1->get_type() == INTEGER || obj1->get_type() == FLOATPOINT) &&
...
throw (runtime_exception_t, std::invalid_argument) {
    ...
    if( !strcmp(op, "++") )
    {
        ...
        assign_t *assign = new assign_t(expr->get_value(), new binary_t("+", expr->get_value(), new value_t("1")));
...
ref_t runtime_t::create_var(var_t* var, ref_t& ref, frame_stack_t& fs) {
    ...
    RUNTIME_DEBUG("Insert var $%s refer to %p to scope", var->get_name().data(), ref);
    return ref;
}

ref_t& runtime_t::get_var_ref(var_t* var, frame_stack_t& fs, ref_t default_ref) throw(runtime_exception_t) {
    ...
    for(; i!= fs.end(); i++)
    {
        ...
        try {
            ...
            if( typeid(*var) == typeid(array_t) )
            {
                ...
                if( array_map->find(key) == array_map->end() && default_ref != 0x0)
                {
...
object_t* runtime_t::get_var_value(var_t* var, frame_stack_t& fs) throw (runtime_exception_t) {
    ...
    RUNTIME_DEBUG("* get_var_value( $%s ), type = %s, value = %s ", var->get_name().data(),
                  dt::type_to_string(obj->get_type()), ((const char*) *obj));
    return obj;
}
